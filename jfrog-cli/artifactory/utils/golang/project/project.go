...
type Go interface {
	...
	DownloadFromVcsAndPublish(targetRepo, goArg, goModEditMessage string, recursiveTidy, recursiveTidyOverwrite bool, details *config.ArtifactoryDetails) error
}
...
func (project *goProject) DownloadFromVcsAndPublish(targetRepo, goArg, goModEditMessage string, recursiveTidy, recursiveTidyOverwrite bool, details *config.ArtifactoryDetails) error {
	wd, err := os.Getwd()
	if err != nil {
...
func collectDependenciesPopulateAndPublish(targetRepo, goModEditMsg, rootProjectDir string, recursiveTidy, recursiveTidyOverwrite bool, cache *golang.DependenciesCache, details *config.ArtifactoryDetails) error {
	...
	if recursiveTidy {
		...
		dependency = &dependencies.PackageWithDeps{GoModEditMessage: goModEditMsg}
		err = dependency.Init()
		if err != nil {
			return err
		}
