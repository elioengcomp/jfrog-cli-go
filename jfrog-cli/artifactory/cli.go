...
func goRecursiveTidyCmd(c *cli.Context) {
	...
	details := createArtifactoryDetailsByFlags(c, true)
	err := goutils.SetGoProxyEnvVar(details, targetRepo)
	if err != nil {
		cliutils.ExitOnErr(err)
	}
	...
	if !exists {
		...
		emptyFile.Close()
		err = goutils.RunGoModTidy("// Generated by GoCenter", true)
		if err != nil {
			cliutils.ExitOnErr(err)
		}
	}
	...
	err = goProject.DownloadFromVcsAndPublish(targetRepo, "", true, c.Bool("recursive-tidy-overwrite"), true, details)
	if err != nil {
		cliutils.ExitOnErr(err)
	}
}
